...
const CANONICAL_DIR = ".ai-tools/rules";
...
export class CanonicalStore {
  ...
  async write(items: RuleDefinition[]): Promise<string[]> {
  ...
  async read(id: string): Promise<RuleDefinition | null> {
    const filePath = join(this.baseDir, `${id}.json`);
    if (!existsSync(filePath)) return null;
    const content = await readFile(filePath, "utf-8");
    return JSON.parse(content) as RuleDefinition;
  }
