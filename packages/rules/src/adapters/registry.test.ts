...
function makeFakeAdapter(id: string, detects: boolean = true): BaseRuleAdapter {
  return {
    id,
    name: `${id} Adapter`,
    nativeSupport: true,
    ...
    generate: async (_rules: RuleDefinition[]) => [] as GeneratedFile[],
    import: async () => [] as RuleDefinition[],
    install: async () => {},
    uninstall: async () => {},
  } as unknown as BaseRuleAdapter;
}

describe("Real RuleAdapterRegistry singleton", () => {
  beforeEach(() => {
    registry.clear();
  });
