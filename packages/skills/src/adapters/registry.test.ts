...
function makeFakeAdapter(id: string, detects: boolean = true): BaseSkillAdapter {
  return {
    id,
    name: `${id} Adapter`,
    nativeSupport: true,
    configDir: `.${id}/prompts`,
    detect: async () => detects,
    generate: async (_skills: SkillDefinition[]) => [] as GeneratedFile[],
    import: async () => [] as SkillDefinition[],
    install: async () => {},
    uninstall: async () => {},
  } as unknown as BaseSkillAdapter;
}
