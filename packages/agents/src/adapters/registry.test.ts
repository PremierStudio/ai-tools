...
function makeFakeAdapter(id: string, detects: boolean = true): BaseAgentAdapter {
  return {
    id,
    name: `${id} Adapter`,
    nativeSupport: true,
    ...
    generate: async (_agents: AgentDefinition[]) => [] as GeneratedFile[],
    import: async () => [] as AgentDefinition[],
    install: async () => {},
    uninstall: async () => {},
  } as unknown as BaseAgentAdapter;
}

describe("Real AgentAdapterRegistry singleton", () => {
  beforeEach(() => {
    registry.clear();
  });
