...
describe("KiroMCPAdapter", () => {
  ...
  describe("metadata", () => {
    ...
    it("has correct config path", () => expect(adapter.configPath).toBe(".kiro/mcp.json"));
  });
  ...
  describe("generate", () => {
    it("generates valid JSON config", async () => {
      ...
      expect(files[0]!.path).toBe(".kiro/mcp.json");
