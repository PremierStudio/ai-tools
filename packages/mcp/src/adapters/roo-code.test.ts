...
import { RooCodeMCPAdapter } from "./roo-code.js";
import type { MCPServerDefinition } from "../types/index.js";

describe("RooCodeMCPAdapter", () => {
  ...
  describe("metadata", () => {
    ...
    it("has correct config path", () => expect(adapter.configPath).toBe(".roo/mcp.json"));
  });
  ...
  describe("generate", () => {
    it("generates valid JSON config", async () => {
      ...
      expect(files[0]!.path).toBe(".roo/mcp.json");
