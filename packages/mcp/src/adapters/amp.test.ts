...
describe("AmpMCPAdapter", () => {
  ...
  describe("metadata", () => {
    ...
    it("has correct config path", () => expect(adapter.configPath).toBe(".amp/mcp.json"));
  });
  ...
  describe("generate", () => {
    it("generates valid JSON config", async () => {
      ...
      expect(files[0]!.path).toBe(".amp/mcp.json");
